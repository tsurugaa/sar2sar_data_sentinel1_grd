...
class log_likelihood(nn.Module):
    ...
    def forward(self, denoised, ref):
        batch_size = denoised.shape[0]
        diff = torch.clamp(denoised - ref, min=-50, max=50)
        coef1 = 2 * (self.M_ - self.m_) * diff
        exp_input = torch.clamp(-diff * 2 * (self.M_ - self.m_), min=-50, max=50)
        coef2 = torch.exp(exp_input)
 
        loss = torch.mean(coef1 + coef2)
        return loss
